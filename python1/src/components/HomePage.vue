<template>
  <div class="home" @wheel="handleWheel" @keydown="handleKeyDown" :style="{ transform: `translateY(-${currentIndex * 100}vh)` }">
    
    <div class="page">
      <div v-if="currentIndex === 0">
        <div class="home-container">
          <div class="stars"></div>
          <h1>Sports Counter</h1>
          <h2>运动计数器</h2>
          <p>基于 Vue 3 制作的简易运动计数器</p>
          <img class="home__sr-img" :src="require('@/assets/bgmain.png')" alt="" draggable="false">
        </div>
      </div>
    </div>
    ...
    <div class="page">
      <div v-if="currentIndex === 1">
        ...
        <div class="stars"></div>
        <ul>
          <HomeCard v-for="(item, index) in homeCardList" :key="index" :id="index" @clickButton="handleCardClick(item.func)">
            <template v-slot:id>{{ item.id }}</template>
    ...
    <!-- 其他页面内容 -->
    <div v-for="(content, index) in filteredContents" :key="index">
      {{ content }}
    </div>
...
export default {
      ...
      homeCardList: [
        {
          id: 'I',
          title: '介绍',
          text: '介绍这个项目的信息，包括制作目的、制作人员，计数器的计数流程和方法',
          func: 'toIntroducePage'
        },
        {
          id: 'A',
          title: '关于',
          text: '关于运动计数器的结构和使用方法，帮助用户快速上手',
          func: 'toAboutPage'
        },
        {
          id: 'U',
          title: '使用',
          text: '使用运动计数器的功能，实现部分运动的计数',
          func: 'toUsingPage'
        }
      ]
    }
  ...
  computed: {
    filteredContents() {
      return this.contents.filter((_, index) => index + 2 === this.currentIndex)
    }
  },
  methods: {
  ...
  mounted() {
    console.log(this.contents)// 检查数组是否有内容
...
.home {
  ...
  will-change: transform; /* 提高变换属性的性能 */
}
...
.home__sr-img {
  width: 50%;
  box-shadow: 0px 0px 20px var(--sa-color-primary-base);
  border-radius: 5px;
