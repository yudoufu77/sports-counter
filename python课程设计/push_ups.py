...
def main():
    ...
    if os.path.exists(output_path):
        count = 1
        while True:
            new_output_file = f'俯卧撑 ({count}).avi'
    ...
    # 打开显示窗口
    cv2.namedWindow('Pushup counter(press key ''q'' to exit)', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Pushup counter(press key ''q'' to exit)', window_width, window_height)
    ...
    while cap.isOpened():
        ...
        if len(detector.lm_list) != 0:
            ...
            cv2.putText(img, f"Time: {int(elapsed_time)}s", (20, 80), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
            cv2.putText(img, f"Avg Speed: {avg_speed:.2f} pushups/min", (20, 120), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
        ...
        cv2.imshow('Pushup counter(press key ''q'' to exit)', img)
