...
@app.route('/api/sit-ups', methods=['GET'])
class PoseDetector:
...
def main():
    ...
    output_file = '仰卧起坐.avi'
    path = os.path.join(output_path, output_file)

    if os.path.exists(output_path):
        count = 1
        while True:
            new_output_file = f'仰卧起坐 ({count}).avi'
    ...
    # 打开显示窗口
    cv2.namedWindow('Situp counter(press key ''q'' to exit)', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Situp counter(press key ''q'' to exit)', window_width, window_height)
    ...
    while cap.isOpened():
        ...
        if len(detector.lm_list) != 0:
            lknee = detector.find_angle(img, 27, 25, 23)
            lhip = detector.find_angle(img, 11, 23, 25)
            rknee = detector.find_angle(img, 28, 26, 24)
            rhip = detector.find_angle(img, 12, 24, 26)

            per = np.interp(lhip, (50, 120), (0, 100))
            bar = np.interp(lhip, (50, 120), (380, 50))
            ...
            cv2.putText(img, f"Time: {int(elapsed_time)}s", (20, 80), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
            cv2.putText(img, f"Avg Speed: {avg_speed:.2f} situps/min", (20, 120), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
        ...
        cv2.imshow('Situp counter(press key ''q'' to exit)', img)
