...
def main():
    ...
    if os.path.exists(output_path):
        count = 1
        while True:
            new_output_file = f'深蹲 ({count}).avi'
    ...
    # 打开显示窗口
    cv2.namedWindow('Squat counter(press key ''q'' to exit)', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Squat counter(press key ''q'' to exit)', window_width, window_height)
    ...
    while cap.isOpened():
        ...
        if len(detector.lm_list) != 0:
            ...
            per = np.interp(lknee, (60, 160), (0, 100))
            bar = np.interp(lknee, (60, 160), (380, 50))
            ...
            cv2.putText(img, f"Time: {int(elapsed_time)}s", (20, 80), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
            cv2.putText(img, f"Avg Speed: {avg_speed:.2f} squats/min", (20, 120), cv2.FONT_HERSHEY_PLAIN, 2,
                        (255, 0, 0), 2)
